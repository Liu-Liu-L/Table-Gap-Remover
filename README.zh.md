...
在 Obsidian 的 **实时预览（Live Preview）** 模式下，隐藏标题下方、表格上方的多余空行。

## 问题

在实时预览中，标题（`# ...`）或任意块后面紧跟表格（`| ... |`）时，两者之间总会渲染出一条额外的空行。这条空行可被点击，且视觉上很干扰。
...
插件注册了一个 CodeMirror 6 `ViewPlugin`，为「位于标题与下一块之间」或「紧贴表格行上方」的空行添加 `rhg-gap-line` 装饰类。再通过 `EditorView.theme` 以最高优先级将这些空行折叠为零高度。
